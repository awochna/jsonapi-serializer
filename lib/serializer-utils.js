...
module.exports = function (collectionName, record, payload, opts) {
    ...
    if (isComplexType(current[attribute])) {
      if (opts && opts.ref) {
        ...
        if (opts && opts.attributes) {
          if (_.isArray(current[attribute])) {
            data = _.map(current[attribute], function (item) {
              return pick(item, opts.attributes);
            });
          } else if (_.isPlainObject(current[attribute])) {
            data = pick(current[attribute], opts.attributes);
          }
        } else {
  ...
  this.serializeRef2 = function (dest, current, attribute, opts) {
    var attributes = [];
